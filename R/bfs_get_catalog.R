...
bfs_get_catalog <- function(language = c("de", "fr", "it", "en")) {
  if (missing(language))
    stop("must choose a language, either 'de', 'fr', 'it' or 'en'", call. = FALSE)
  ...
  feed <-
    paste0(
      ...
      language,
      "/home/statistiken/kataloge-datenbanken/daten/_jcr_content/par/ws_catalog.rss.xml?skipLimit=true"
    )
